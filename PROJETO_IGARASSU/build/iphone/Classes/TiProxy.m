...
//Rare exceptions to indicate a bug in the _projeto_igarassu code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_projeto_igarassu = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_projeto_igarassu]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.projeto_igarassu.module"
